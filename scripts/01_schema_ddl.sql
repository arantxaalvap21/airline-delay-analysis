...
-- Campos añadidos: month_name, day_of_week_name, is_weekend,
--   dep_time_blk, banda_dia — necesarios para queries analíticas
--   (agrupar por nombre de mes, filtrar fines de semana, etc.)
-- ============================================================

CREATE TABLE dim_fecha (
    fecha_key        INTEGER      PRIMARY KEY,   -- smart key YYYYMMDD
    fecha            DATE         NOT NULL,
    ...
    day_of_week      INTEGER,
    day_of_week_name VARCHAR(12)  NOT NULL,       -- 'Monday' ... 'Sunday'
    is_weekend       BOOLEAN      NOT NULL,       -- TRUE si day_of_week IN (6,7)
    dep_time_blk     VARCHAR(12)  NOT NULL,       -- bloque horario: '0600-0659' ...
    banda_dia        VARCHAR(12)  NOT NULL        -- 'Morning' | 'Afternoon' | 'Night' | 'Red-eye'
);

-- ============================================================
-- Dimensión Aerolínea
-- Grano: una fila por aerolínea operadora
-- Campo añadido: airline_name — nombre completo para labels
--   en dashboard (en vez de mostrar solo 'AA' o 'DL')
-- ============================================================
...
-- WHERE table_schema = 'airline_dwh' ORDER BY table_name;
--
-- Esperado: dim_aerolinea, dim_aeropuerto_destino,
--           dim_aeropuerto_origen, dim_cancelacion,
--           dim_fecha, dim_ruta, fact_vuelos
-- ============================================================
